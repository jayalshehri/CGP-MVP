...
-- 20260922000000_review_schedule_open_cycle_guard.sql.
--
-- The original file's SQL is a single guard clause added to
-- private.grc_command's `open_cycle` branch -- no new table, column,
-- trigger, or RLS policy, and (unlike Phase 1's migration) no new function
-- either: this is a pure CREATE OR REPLACE of the same existing function.
-- Verified byte-for-byte: everything except the one added guard line below
-- is identical to Production's currently deployed private.grc_command,
-- confirmed via a read-only diff against Production during this
-- release-preparation review. The only reason this is a separate file
-- rather than applying 20260922000000 as-is is that file's header frames
-- itself as "QA-ONLY" / "Phase 2 current-state assessment", which is
-- investigative framing that shouldn't sit in Production's migration
-- history. The historical QA-only 20260922000000 file is left exactly as
-- written -- this is a new, separate file, not an edit of it.
--
-- Problem this closes: private.grc_command's `open_cycle` branch had no
-- guard against opening a second review cycle for a control that already
-- has one open. The client (/audit-schedule) already avoids this in the
-- happy path, but nothing in the database itself prevented a second
-- open_cycle call for the same control -- a stale client, a race between
-- two admins, or a direct RPC call could have created two open cycles (and
-- two evidence_requests) for one control.
--
-- This migration does NOT seed any schedule, does NOT alter any existing
-- owner, does NOT create any cycle/evidence_request/assessment, does NOT
-- change any compliance state, and does NOT alter any historical review
-- date. It only closes the duplicate-open-cycle gap. 'reschedule' already
-- requires an existing open cycle (cycle.status<>'open' raises), and
-- 'assess' already flips the cycle to 'completed' before opening the next
-- one, so neither of those paths could ever produce a duplicate open cycle
-- -- 'open_cycle' was the only gap, and is the only branch touched.
--
-- Idempotent: CREATE OR REPLACE FUNCTION is safe to re-run. No naming
-- collision risk -- this migration introduces zero new database objects.
