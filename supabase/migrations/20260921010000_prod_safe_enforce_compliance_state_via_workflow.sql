-- Production-safe counterpart to
-- 20260921000000_enforce_compliance_state_via_workflow.sql.
--
-- The original file's SQL is schema/workflow-enforcement only -- no QA IDs,
-- no QA users, no pilot data, no evidence data, no owner assignments, no
-- control status seed. Verified byte-for-byte: the private.grc_command body
-- below (everything except the single added set_config line) is identical
-- to Production's currently deployed private.grc_command, confirmed via a
-- read-only diff against Production during release-preparation review. The
-- only reason this is a separate file rather than applying 20260921000000
-- as-is is that file's header frames itself as "QA-ONLY" / "Phase 1 Pilot",
-- which is investigative framing, not something that belongs in Production's
-- migration history. The historical QA-only 20260921000000 file is left
-- exactly as written -- this is a new, separate file, not an edit of it.
--
-- Problem this closes: the cgp_admin_team_update_controls UPDATE policy on
-- public.controls has no column restriction, so any admin/cybersecurity_team
-- account could set implementation_status/verification_status directly via
-- the API, bypassing the governed assess workflow (independent reviewer,
-- accepted/valid/non-self-uploaded evidence, resolved requests, rationale).
-- The pre-existing assessment_control_guard trigger only blocks this for
-- CSCC/DCC/TCC/OSMACC controls (a separate framework-level assessment
-- subsystem's own guard) -- it does not cover ECC or CCC, where the real
-- per-control review-cycle workflow lives. This migration closes that gap
-- for every framework, without touching the existing guard.
--
...
-- connections, or pooled sessions, and nothing needs to explicitly clear it.
--
-- No RLS policy, grant, constraint, or enum/check value is touched. No
-- Production business data is mutated. Preserves every existing
-- function/trigger unchanged: grc_audit, assessment_control_guard,
-- cgp_prepare_evidence, cgp_sync_evidence, grc_refresh_evidence_summary,
-- grc_expire_evidence_sweep are not modified. private.grc_command is
-- replaced with its existing Production body plus exactly one added line
-- (the set_config call) -- no other logic changes.
--
-- Idempotent: CREATE OR REPLACE FUNCTION and DROP TRIGGER IF EXISTS / CREATE
-- TRIGGER are both safe to re-run. Confirmed via read-only inspection that
-- neither private.enforce_compliance_state_via_workflow nor a trigger named
-- enforce_compliance_state exists anywhere on Production prior to this
-- migration -- no naming collision.
