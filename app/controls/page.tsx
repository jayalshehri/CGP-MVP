 ...
 return <main className="workflow-page catalog-page" dir="rtl">
  ...
  {selectedFramework&&<>
   ...
   <section aria-labelledby="domain-heading">
    ...
    <div className="domain-grid">{[...domains].map(([name,rows])=>{const done=rows.filter(c=>good(c.implementation_status)).length;const percent=rows.length?Math.round(done/rows.length*100):0;const parentCount=rows.filter(c=>c.hierarchy_level!=="sub_control").length;const subCount=rows.filter(c=>c.hierarchy_level==="sub_control").length;const subdomainCount=new Set(rows.map(c=>c.control_code.split("-").slice(0,2).join("-"))).size;const stats=[parentCount>0?`${parentCount} ضابط رئيسي`:null,subCount>0?`${subCount} ضابط فرعي`:null,subdomainCount>0?`${subdomainCount} ${subdomainCount===1?"مجال فرعي":"مجالات فرعية"}`:null].filter(Boolean).join(" · ");return <button key={name} className={`domain-card ${selectedDomain===name?"active":""}`} onClick={()=>setActiveDomain(name)} aria-pressed={selectedDomain===name}><span className="domain-index">{domainNumber(rows)}</span><span><strong>{name}</strong><small>{stats}</small></span><b>{percent}%</b></button>})}</div>
   </section>
