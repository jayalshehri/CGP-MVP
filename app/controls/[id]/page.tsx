...
export default function ControlDetailsPage() {
 ...
 return <main className="detail-page" dir="rtl">
  ...
  <section id={`detail-panel-${tab}`} role="tabpanel" aria-labelledby={`detail-tab-${tab}`} className="detail-columns">
   ...
   {tab===0&&<><section className="detail-card"><h2>المتطلب الرسمي</h2><p className="detail-official">{strategyExample?.requirement||control.description_ar||'لا يوجد وصف مسجل.'}</p>{plan.requirements.length>0&&<><h3>المتطلبات الفرعية</h3>{plan.requirements.map(r=><p className="detail-requirement" key={r.key}><b dir="ltr">{r.key}</b> {r.text}</p>)}</>}<a className="detail-back" href={strategyExample?eccOfficialControlsUrl:`https://nca.gov.sa/ar/regulatory-documents/controls-list/${frameworkCode}/`} target="_blank" rel="noreferrer">مرجع الضوابط الرسمية للهيئة ↗</a></section>{strategyExample?<section className="detail-card"><h2>الإرشاد المرجعي للهيئة</h2><p>يوجد دليل إرشادي مستقل لتطبيق ECC. يُستخدم للاستئناس بمنهج التطبيق، بينما يبقى نص ECC الحالي هو المتطلب الملزم عند اختلاف الإصدار أو الصياغة.</p><a className="detail-back" href={eccImplementationGuideUrl} target="_blank" rel="noreferrer">فتح الدليل الإرشادي ↗</a></section>:<section className="detail-card"><h2>إرشادات التطبيق المقترحة</h2><p>خطة مساعدة لتنفيذ «{control.title_ar}» وتوثيق الأدلة التي تثبت استيفاء نص الضابط. تُراجع بحسب نطاق الجهة؛ وليست نصًا تنظيميًا إضافيًا.</p><button className="detail-button" onClick={()=>setTab(1)}>عرض خطوات التنفيذ ←</button></section>}</>}
   {tab===1&&<section className="detail-card"><h2>{strategyExample?'خطة تنفيذ الجهة':'قائمة إجراءات التنفيذ'}</h2><p className="detail-hint">{strategyExample?'هذه خطة داخلية قابلة للتخصيص للجهة؛ وليست نصًا إلزاميًا من الهيئة.':'حفظ الخطوات يساعد على المتابعة. اعتماد الدليل والتحقق يتمان عبر دورة المراجعة.'}</p>{executionSteps.map(step=><label className="detail-step" key={step.key}><input type="checkbox" checked={!!completed[step.key]} disabled={saving} onChange={()=>toggle(step.key)}/><span>{step.text}</span><small>{completed[step.key]?'مكتمل':'لم يكتمل'}</small></label>)}</section>}
