...
export default function ControlDetailsPage() {
 ...
 return <main className="detail-page" dir="rtl">
  ...
  <header className="detail-hero"><div><span className="detail-code">{control.control_code}</span><h1>{getEccOfficialTitle(control.control_code)||strategyExample?.title||control.title_ar}</h1><p>{control.domain_ar}</p></div><div className="detail-hero-actions"><StatusBadge status={control.implementation_status}/>{canReview&&<Link className="detail-assign-owner" href={`/controls/${control.id}/assign`}>{control.control_owner?"تغيير مالك الضابط":"تعيين مالك الضابط"} ←</Link>}</div></header>
  ...
  <section id={`detail-panel-${tab}`} role="tabpanel" aria-labelledby={`detail-tab-${tab}`} className="detail-columns">
   ...
   {tab===1&&<><section className="detail-card"><h2>الأدلة المقترحة</h2><p className="detail-hint">أمثلة مساعدة؛ تُحدد كفايتها وفق نص الضابط ونطاق التطبيق.</p><ul>{plan.evidence.map(item=><li key={item}>{item}</li>)}</ul>{canUpload&&<Link className="detail-button" href={`/controls/${control.id}/evidence/new`}>+ رفع دليل</Link>}</section><section className="detail-card"><h2>الأدلة والإصدارات</h2>{!evidence.length&&<p>لا توجد أدلة مرفوعة بعد.</p>}{evidence.map(e=><article className="control-evidence-item" key={e.id}><small>{e.is_current?'الإرسال الحالي':'إصدار سابق'}</small><h3>{e.evidence_name||e.file_name}</h3><StatusBadge status={e.status||''}/>{e.uploaded_at&&<p>{date(e.uploaded_at)}</p>}{e.description&&<p>{e.description}</p>}{e.review_notes&&<p>ملاحظات المراجع: {e.review_notes}</p>}<EvidenceDownload path={e.file_path} name={e.file_name}/>{canReview&&e.is_current&&['pending_review','under_review'].includes(e.status||'')&&<Link className="detail-button" href="/review">مراجعة الدليل</Link>}</article>)}</section></>}
   {tab===2&&<section className="detail-card"><h2>الضوابط المرتبطة</h2><p className="detail-hint">هذه مواءمات صريحة مستخرجة من مراجع NCA. يمكن رفع الدليل مرة واحدة من ضابط ECC ثم اختياره للضوابط المرتبطة؛ ويظل قرار القبول مستقلًا لكل ضابط.</p>{!canReview?<p>تظهر المواءمات التفصيلية لمدير الامتثال والمراجعة ومدير النظام.</p>:!mappedControls.length?<p>لا توجد مواءمات رسمية مسجلة لهذا الضابط.</p>:<div className="detail-timeline">{mappedControls.map(item=>{const reflection=reflections.find(result=>result.source_framework===item.framework_code&&result.source_control_code===item.control_code);return <article key={item.control_id}><strong><span dir="ltr">{item.framework_code} · {item.control_code}</span> — {item.control_title}</strong><small>{reflection?`نتيجة الأداة: ${assessmentStatusText[reflection.compliance_status]||'لم يُقيّم'}`:'لم تُسجل نتيجة تقييم بعد'}</small><p>{item.source_note||'مواءمة مرجعية رسمية.'}</p><Link className="detail-back" href={`/controls/${item.control_id}`}>فتح الضابط المرتبط ←</Link></article>})}</div>}{canReview&&<p style={{marginTop:18}}><Link className="detail-button" href="/mappings">فتح خريطة المواءمة ←</Link></p>}</section>}
