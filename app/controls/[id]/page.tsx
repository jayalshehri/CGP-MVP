...
export default function ControlDetailsPage() {
 ...
 return <main className="detail-page" dir="rtl">
  ...
  <section id={`detail-panel-${tab}`} role="tabpanel" aria-labelledby={`detail-tab-${tab}`} className="detail-columns">
   ...
   {tab===0&&<><section className="detail-card"><h2>المتطلب الرسمي</h2><p className="detail-official">{officialRequirement}</p>{plan.requirements.length>0&&<><h3>المتطلبات الفرعية</h3>{plan.requirements.map(r=><p className="detail-requirement" key={r.key}><b dir="ltr">{r.key}</b> {r.text}</p>)}</>}<a className="detail-back" href={isEcc?eccOfficialControlsUrl:`https://nca.gov.sa/ar/regulatory-documents/controls-list/${frameworkCode}/`} target="_blank" rel="noreferrer">مرجع الضوابط الرسمية للهيئة ↗</a></section>{isEcc&&<section className="detail-card"><h2>الإرشاد المرجعي للهيئة</h2><p>يوجد دليل إرشادي مستقل لتطبيق ECC. يُستخدم للاستئناس بمنهج التطبيق، بينما يبقى نص ECC الحالي هو المتطلب الملزم عند اختلاف الإصدار أو الصياغة.</p><a className="detail-back" href={eccImplementationGuideUrl} target="_blank" rel="noreferrer">فتح الدليل الإرشادي ↗</a></section>}<section className="detail-card"><h2>طريقة استخدام المنصة</h2><p>استخدم خطة تنفيذ الجهة لتوزيع العمل ومتابعة الإنجاز، ثم ارفع الأدلة واطلب مراجعتها. لا تغيّر هذه الخطوات نص المتطلب الرسمي أو قرار قبول الدليل.</p><button className="detail-button" onClick={()=>setTab(1)}>فتح خطة تنفيذ الجهة ←</button></section></>}
