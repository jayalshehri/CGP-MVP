...
type Control = { id:number; control_code:string; title_ar:string; description_ar:string|null; official_text_ar:string|null; hierarchy_level:string; parent_control_id:number|null; applicability:string|null; source_page:string|null; domain_ar:string; implementation_status:string; evidence_status:string; verification_status:string; due_date:string|null; last_review_date:string|null; control_owner:string|null; evidence_owner:string|null; audit_frequency:string; next_audit_date:string|null; frameworks:{code:string,name_ar:string,version:string,is_active:boolean,source_url:string|null}|null; };
...
const tabs=['نظرة عامة','الأدلة المطلوبة','الضوابط المرتبطة','النشاط'];
...
export default function ControlDetailsPage() {
 ...
 return <main className="detail-page" dir="rtl">
  ...
  <div className="detail-metrics"><section><small>حالة التطبيق</small><StatusBadge status={control.implementation_status}/></section><section><small>حالة الدليل</small><StatusBadge status={control.evidence_status}/></section><section><small>حالة التحقق</small><StatusBadge status={control.verification_status}/></section><section><small>موعد الاستحقاق</small><strong>{control.due_date?formatComplianceDate(control.due_date):'غير محدد'}</strong></section></div>
  {!archived&&<section className="detail-next"><div><span>{canUpload?'الإجراء التالي':'وضع المراجعة'}</span><strong>{canUpload?(control.evidence_status==='not_uploaded'?'رفع الدليل المطلوب للضابط':!goodStatus(control.verification_status)?'متابعة مراجعة الدليل والتحقق':'مراجعة الضابط دوريًا والمحافظة على الأدلة'):'تستطيع معاينة الدليل والسجل فقط ضمن نطاق التدقيق الممنوح لك.'}</strong></div>{canUpload&&control.evidence_status==='not_uploaded'?<Link className="detail-button" href={`/controls/${control.id}/evidence/new`}>رفع دليل ←</Link>:<button className="detail-button" onClick={()=>setTab(3)}>عرض النشاط ←</button>}</section>}
  <section className="detail-schedule-strip">
   ...
   <div><small>آخر مراجعة</small><strong>{formatComplianceDate(control.last_review_date)}</strong></div>
   <div><small>المراجعة القادمة</small><strong>{formatComplianceDate(control.next_audit_date)}</strong><span className={`audit-state ${scheduleState(control.next_audit_date)}`}>{scheduleStateLabels[scheduleState(control.next_audit_date)]}</span></div>
   <div><small>مرحلة الدورة الحالية</small><span className={`audit-stage ${cycleStage(!!openCycleId,latestRequestStatus)}`}>{cycleStageLabels[cycleStage(!!openCycleId,latestRequestStatus)]}</span>{openCycleDue&&<small className="detail-schedule-due">حتى {formatComplianceDate(openCycleDue)}</small>}</div>
  ...
  <section id={`detail-panel-${tab}`} role="tabpanel" aria-labelledby={`detail-tab-${tab}`} className="detail-columns">
   ...
   {tab===3&&<section className="detail-card"><h2>النشاط الأخير</h2><p className="detail-hint">آخر عمليات رفع الأدلة وقرارات مراجعتها لهذا الضابط. تبقى جميع الإصدارات والقرارات في سجل الأدلة.</p>{!timeline.length&&<p>لا توجد أنشطة مسجلة بعد.</p>}<ol className="detail-timeline">{timeline.slice(0,5).map(item=><li key={item.key}><strong>{item.title}</strong><small>{date(item.time)}</small>{item.body&&<p>{item.body}</p>}</li>)}</ol>{canViewAudit&&<Link className="detail-back" href="/audit">عرض سجل النشاط الكامل ←</Link>}{!canViewAudit&&<p className="detail-hint">السجل التقني الكامل متاح للفريق المخوّل من صفحة سجل النشاط والتغييرات.</p>}</section>}
   </div>
   <aside>{tab===0&&!archived&&<section className="detail-card"><h2>التدقيق الدوري</h2><p className="detail-hint">تتم جدولة ومتابعة جميع التدقيقات من صفحة موحدة حسب سياسة الجهة.</p><p><strong>الموعد القادم:</strong> {control.next_audit_date?formatComplianceDate(control.next_audit_date):"غير مجدول"}</p><Link className="detail-button" href="/audit-schedule">فتح جدول التدقيق الدوري ←</Link></section>}</aside>
  </section>
 </main>;
}
