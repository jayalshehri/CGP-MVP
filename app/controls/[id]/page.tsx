...
const tabs=['نظرة عامة','خطة تنفيذ الجهة','الأدلة المطلوبة','الضوابط المرتبطة','السجل والمراجعات'];
...
export default function ControlDetailsPage() {
 ...
 const [auditFrequency,setAuditFrequency]=useState('annual'),[nextAuditDate,setNextAuditDate]=useState(''),[lastAuditDate,setLastAuditDate]=useState('');
 ...
 useEffect(()=>{let active=true;(async()=>{try{
  ...
  if(active){setControl(c.data);setAuditFrequency(c.data.audit_frequency||'annual');setNextAuditDate(c.data.next_audit_date||'');setLastAuditDate(c.data.last_audit_date||'');setEvidence(e.data??[]);setCompleted(Object.fromEntries((w.data??[]).map(x=>[x.item_key,x.completed])));setNotes(n.data??[]);}
 ...
 const executionSteps=strategyExample
  ? strategyExample.internalSteps.map((text,index)=>({key:`strategy-${control.control_code}-${index}`,text}))
  : plan.steps;
 const frameworkCode=(control.frameworks?.code||'ECC').toLowerCase();
 const done=executionSteps.filter(s=>completed[s.key]).length,percent=Math.round(done/executionSteps.length*100);
 ...
 return <main className="detail-page" dir="rtl">
  ...
  <header className="detail-hero"><div><span className="detail-code">{control.control_code}</span><h1>{strategyExample?.title||control.title_ar}</h1><p>{control.domain_ar} · المالك: {control.control_owner||'غير محدد'}</p></div><StatusBadge status={control.implementation_status}/></header>
  ...
  <div className="detail-metrics"><section><small>تقدم خطة التنفيذ</small><strong>{percent}% · {done} من {executionSteps.length}</strong><progress max={executionSteps.length} value={done} aria-label="تقدم خطة التنفيذ"/></section><section><small>حالة الدليل</small><StatusBadge status={control.evidence_status}/></section><section><small>حالة التحقق</small><StatusBadge status={control.verification_status}/></section><section><small>موعد الاستحقاق</small><strong>{control.due_date||'غير محدد'}</strong></section></div>
  ...
  <section id={`detail-panel-${tab}`} role="tabpanel" aria-labelledby={`detail-tab-${tab}`} className="detail-columns">
   ...
   {tab===0&&<><section className="detail-card"><h2>المتطلب الرسمي</h2><p className="detail-official">{strategyExample?.requirement||control.description_ar||'لا يوجد وصف مسجل.'}</p>{plan.requirements.length>0&&<><h3>المتطلبات الفرعية</h3>{plan.requirements.map(r=><p className="detail-requirement" key={r.key}><b dir="ltr">{r.key}</b> {r.text}</p>)}</>}<a className="detail-back" href={isEcc?eccOfficialControlsUrl:`https://nca.gov.sa/ar/regulatory-documents/controls-list/${frameworkCode}/`} target="_blank" rel="noreferrer">مرجع الضوابط الرسمية للهيئة ↗</a></section>{isEcc&&<section className="detail-card"><h2>الإرشاد المرجعي للهيئة</h2><p>يوجد دليل إرشادي مستقل لتطبيق ECC. يُستخدم للاستئناس بمنهج التطبيق، بينما يبقى نص ECC الحالي هو المتطلب الملزم عند اختلاف الإصدار أو الصياغة.</p><a className="detail-back" href={eccImplementationGuideUrl} target="_blank" rel="noreferrer">فتح الدليل الإرشادي ↗</a></section>}<section className="detail-card"><h2>طريقة استخدام المنصة</h2><p>استخدم خطة تنفيذ الجهة لتوزيع العمل ومتابعة الإنجاز، ثم ارفع الأدلة واطلب مراجعتها. لا تغيّر هذه الخطوات نص المتطلب الرسمي أو قرار قبول الدليل.</p><button className="detail-button" onClick={()=>setTab(1)}>فتح خطة تنفيذ الجهة ←</button></section></>}
   {tab===1&&<section className="detail-card"><h2>خطة تنفيذ الجهة</h2><p className="detail-hint">هذه خطة داخلية قابلة للتخصيص للجهة؛ وليست نصًا إلزاميًا من الهيئة. اعتماد الدليل والتحقق يتمان عبر دورة المراجعة.</p>{executionSteps.map(step=><label className="detail-step" key={step.key}><input type="checkbox" checked={!!completed[step.key]} disabled={saving} onChange={()=>toggle(step.key)}/><span>{step.text}</span><small>{completed[step.key]?'مكتمل':'لم يكتمل'}</small></label>)}</section>}
   ...
   <aside><section className="detail-card"><h2>المسؤوليات</h2><dl><dt>مالك الضابط</dt><dd>{control.control_owner||'غير محدد'}</dd><dt>مسؤول الدليل</dt><dd>{control.evidence_owner||'غير محدد'}</dd><dt>حالة التنفيذ</dt><dd>{statusText(control.implementation_status)}</dd><dt>آخر مراجعة</dt><dd>{control.last_review_date||'لم تتم المراجعة'}</dd></dl>{canAssign&&<Link className="detail-button" href={`/controls/${control.id}/assign`}>تعديل المالك والاستحقاق</Link>}</section>{tab===0&&<section className="detail-card"><h2>جدول التدقيق الدوري</h2><p className="detail-hint">الإعداد الافتراضي سنوي، ويمكن تقصير المدة حسب مستوى الخطورة أو متطلبات الجهة.</p><label htmlFor="audit-frequency">تكرار التدقيق</label><select id="audit-frequency" value={auditFrequency} onChange={e=>setAuditFrequency(e.target.value)} disabled={!canAssign||saving} style={{display:'block',width:'100%',margin:'8px 0 14px',padding:11,border:'1px solid #c5d1d7',borderRadius:8,font:'inherit'}}><option value="monthly">شهري</option><option value="quarterly">ربع سنوي</option><option value="semiannual">نصف سنوي</option><option value="annual">سنوي</option><option value="custom">مخصص</option></select><label htmlFor="last-audit-date">تاريخ آخر تدقيق</label><input id="last-audit-date" type="date" value={lastAuditDate} onChange={e=>setLastAuditDate(e.target.value)} disabled={!canAssign||saving} style={{display:'block',width:'100%',boxSizing:'border-box',margin:'8px 0 14px',padding:11,border:'1px solid #c5d1d7',borderRadius:8,font:'inherit'}}/><label htmlFor="next-audit-date">موعد التدقيق القادم</label><input id="next-audit-date" type="date" value={nextAuditDate} onChange={e=>setNextAuditDate(e.target.value)} disabled={!canAssign||saving} style={{display:'block',width:'100%',boxSizing:'border-box',margin:'8px 0 14px',padding:11,border:'1px solid #c5d1d7',borderRadius:8,font:'inherit'}}/>{canAssign&&<button className="detail-button" disabled={saving} onClick={saveAuditSchedule}>{saving?'جاري الحفظ…':'حفظ جدول التدقيق'}</button>}</section>}<section className="detail-card"><h2>ملاحظات التنفيذ</h2>{control.implementation_notes&&<p>{control.implementation_notes}</p>}<label htmlFor="control-note">إضافة ملاحظة</label><textarea id="control-note" rows={4} maxLength={4000} value={draft} onChange={e=>setDraft(e.target.value)} placeholder="وثّق ما تم تنفيذه أو العوائق…"/><button className="detail-button" disabled={saving||!draft.trim()} onClick={addNote}>{saving?'جاري الحفظ…':'حفظ الملاحظة'}</button>{notes.length>0&&<p className="detail-hint">{notes.length} ملاحظة محفوظة في السجل.</p>}</section></aside>
  </section>
 </main>;
}
