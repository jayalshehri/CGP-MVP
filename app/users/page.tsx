...
export default function UsersPage() {
  ...
  async function loadProfiles() {
    const { data, error: loadError } = await supabase.from("profiles").select("user_id,display_name,role,is_active,created_at,updated_at").order("created_at", { ascending: true });
    if (loadError) { setError("تعذر تحميل المستخدمين."); return; }
  ...
  return <main dir="rtl" style={{ minHeight: "100vh", background: "#f5f7f9", color: "#0b1f33", fontFamily: "Arial" }}><section className="cgp-page-body" style={{ padding: 40, maxWidth: 1180, margin: "0 auto" }}>
    <div style={{ display: "flex", justifyContent: "space-between", gap: 20, flexWrap: "wrap", marginBottom: 24 }}><div><h1>إدارة المستخدمين</h1><p style={{ color: "#71808e" }}>دعوة المستخدمين وإدارة الأدوار والتفعيل.</p></div><div style={{ display: "flex", flexWrap: "wrap", gap: 10, minWidth: 0 }}><input aria-label="البحث عن مستخدم" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="بحث بالاسم أو الدور" style={inputStyle}/><button onClick={() => setShowInvite((value) => !value)} style={primary}>+ إضافة مستخدم</button></div></div>
    ...
    {error && <div role="alert" style={err}>{error}</div>}{success && <div role="status" style={ok}>{success}</div>}{activationLink && <div style={{ ...ok, overflowWrap: "anywhere" }}><a href={activationLink} target="_blank" rel="noreferrer" style={{ fontWeight: 800, color: "#0f6f67" }}>فتح رابط التفعيل الجديد</a><div style={{ fontSize: 12, marginTop: 8 }}>يمكنك فتح الرابط مباشرة على جهاز المستخدم.</div></div>}
    <div style={{ background: "white", borderRadius: 14, overflow: "hidden" }}><div className="cgp-table-scroll" role="region" aria-label="جدول المستخدمين" tabIndex={0}><table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr><th style={th}>المستخدم</th><th style={th}>الدور</th><th style={th}>الحالة</th><th style={th}>التفعيل</th></tr></thead><tbody>{filteredProfiles.map((profile) => { const isSelf = profile.user_id === currentUserId; return <tr key={profile.user_id} style={{ borderTop: "1px solid #edf0f2" }}><td style={td}><b>{profile.display_name || "بدون اسم"}{isSelf ? " (أنت)" : ""}</b><div style={{ fontSize: 12, color: "#586875" }}>{profile.user_id.slice(0, 8)}…</div></td><td style={td}><select aria-label={`دور ${profile.display_name || "المستخدم"}`} value={profile.role} disabled={savingId === profile.user_id || isSelf} onChange={(event) => updateProfile(profile.user_id, { role: event.target.value as UserRole })} style={inputStyle}><option value="admin">مدير النظام</option><option value="cybersecurity_team">فريق الأمن السيبراني</option><option value="control_owner">مالك الضابط</option></select></td><td style={td}><button disabled={savingId === profile.user_id || isSelf} onClick={() => updateProfile(profile.user_id, { is_active: !profile.is_active })}>{profile.is_active ? "نشط" : "موقوف"}</button></td><td style={td}><button disabled={savingId === profile.user_id} onClick={() => resend(profile.user_id)} style={secondary}>إعادة رابط التفعيل</button></td></tr>; })}</tbody></table></div></div>
  </section></main>;
}
