...
export default function UsersPage() {
  ...
  return <main dir="rtl" className="workflow-page"><section>
    ...
    <div className="workflow-metrics user-metrics"><WorkflowMetric label="إجمالي المستخدمين" value={profiles.length}/><WorkflowMetric label="الحسابات النشطة" value={profiles.filter(p=>p.is_active).length} tone="success"/><WorkflowMetric label="فريق الأمن السيبراني" value={profiles.filter(p=>p.role==="cybersecurity_team").length}/><WorkflowMetric label="فريق إدارة البيانات" value={profiles.filter(p=>p.role==="data_governance_team").length}/><WorkflowMetric label="مالكو الضوابط" value={profiles.filter(p=>p.role==="control_owner").length}/></div>
    ...
    <div style={{ background: "white", borderRadius: 14, overflow: "hidden" }}><div className="cgp-table-scroll" role="region" aria-label="جدول المستخدمين" tabIndex={0}><table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr><th style={th}>المستخدم</th><th style={th}>الدور</th><th style={th}>الحالة</th><th style={th}>التفعيل</th></tr></thead><tbody>{filteredProfiles.map((profile) => { const isSelf = profile.user_id === currentUserId; return <tr key={profile.user_id} style={{ borderTop: "1px solid #edf0f2" }}><td style={td}><b>{profile.display_name || "بدون اسم"}{isSelf ? " (أنت)" : ""}</b><div style={{ fontSize: 12, color: "#586875" }}>{profile.user_id.slice(0, 8)}…</div></td><td style={td}><select aria-label={`دور ${profile.display_name || "المستخدم"}`} value={profile.role} disabled={savingId === profile.user_id || isSelf} onChange={(event) => updateProfile(profile.user_id, { role: event.target.value as UserRole })} style={inputStyle}><option value="admin">مدير النظام</option><option value="cybersecurity_team">فريق الأمن السيبراني</option><option value="data_governance_team">فريق إدارة البيانات والحوكمة</option><option value="control_owner">مالك الضابط</option></select></td><td style={td}><button disabled={savingId === profile.user_id || isSelf} onClick={() => updateProfile(profile.user_id, { is_active: !profile.is_active })}>{profile.is_active ? "نشط" : "موقوف"}</button></td><td style={td}><button disabled={savingId === profile.user_id} onClick={() => resend(profile.user_id)} style={secondary}>إعادة رابط التفعيل</button></td></tr>; })}</tbody></table></div></div>
  </section></main>;
}
