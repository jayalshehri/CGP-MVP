...
{error&&<div role="alert" style={{background:"#fff2f0",color:"#9d2e24",padding:14,borderRadius:10,marginBottom:16}}>{error}</div>}
      {message&&<div role="status" style={{background:"#e8f5f2",color:"var(--cgp-teal-dark)",padding:14,borderRadius:10,marginBottom:16}}>{message}</div>}
      ...
      {visiblePending.length===0?<div style={empty}>{pending.length?"لا توجد أدلة مطابقة للبحث.":"لا توجد أدلة بانتظار المراجعة."}</div>:visiblePending.map(row=><div key={row.id} className="review-card" style={card}>
        ...
        <label className="cgp-field-label" style={{marginTop:18}} htmlFor={`review-notes-${row.id}`}>ملاحظات المراجع — مطلوبة عند الرفض</label><textarea id={`review-notes-${row.id}`} value={notes[row.id]||""} onChange={e=>setNotes({...notes,[row.id]:e.target.value})} aria-label="ملاحظات المراجعة" placeholder="ملاحظات المراجع (مطلوبة عند الرفض)" rows={3} style={{width:"100%",boxSizing:"border-box",marginTop:0,border:"1px solid #ccd6dc",borderRadius:10,padding:12,fontFamily:"inherit",resize:"vertical"}}/>
