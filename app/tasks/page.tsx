...
<WorkflowHeading title={role==="control_owner"?"مهامي":"التكليفات"} description="راجع الاستحقاقات، جهّز الأدلة، وتابع حالة كل ضابط." action={<Link className="workflow-button" href="/controls">سجل الضوابط ←</Link>}/>
        ...
        <div className="cgp-responsive-grid cgp-kpi-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:16,marginBottom:22}}>
          <Kpi label="إجمالي التكليفات" value={tasks.length}/><Kpi label="مكتملة" value={completed} tone="success"/><Kpi label="بانتظار أدلة" value={waitingEvidence} tone={waitingEvidence?"warning":"neutral"}/><Kpi label="متأخرة" value={overdue} tone={overdue?"danger":"neutral"}/>
        </div>
        ...
        <ResultSummary count={filtered.length} total={tasks.length} active={!!search||filter!=="all"} reset={()=>{setSearch("");setFilter("all");}}/>
<div style={{background:"white",border:"1px solid #e2e7eb",borderRadius:14,overflow:"hidden"}}>
          {filtered.length===0?<div style={{padding:45,textAlign:"center",color:"#586875"}}>لا توجد تكليفات مطابقة حاليًا.</div>:filtered.map(task=>{
            ...
            return <div className="workflow-row cgp-responsive-grid" key={task.id} style={{padding:20,borderBottom:"1px solid #edf0f2",display:"grid",gridTemplateColumns:"1.6fr .8fr .8fr .8fr auto",gap:15,alignItems:"center"}}>
