...
export default function ExecutivePage() {
  ...
  return <main dir="rtl" className="exec-page" aria-busy={loading}>
    ...
    {error ? <section className="exec-panel exec-empty" role="alert"><h2>تعذر عرض البيانات</h2><p>{error}</p><button onClick={refresh}>إعادة المحاولة</button></section> : loading ? <section className="exec-panel exec-empty" role="status">جاري إعداد الملخص التنفيذي…</section> : summary.total === 0 ? <section className="exec-panel exec-empty"><h2>يبدأ الملخص بإضافة الضوابط</h2><p>ستظهر نسب التنفيذ والتحقق والأولويات عند توفر بيانات ضمن نطاقك.</p><Link href="/controls">فتح الضوابط ←</Link></section> : <>
      ...
      <section className="exec-visual-grid" aria-label="تحليل تنفيذي مرئي">
        <section className="exec-visual-card">
          <div className="exec-visual-heading"><div><span className="exec-eyebrow">الأداء حسب المجال</span><h2>مسار الالتزام</h2></div><Link href={`/reports${framework!=="all"?`?framework=${framework}`:""}`}>عرض التقارير ←</Link></div>
          <div className="exec-domain-chart">
            {summary.domains.slice(0,6).map(domain=>{const value=percent(domain.done,domain.total);return <Link key={domain.name} href={`/controls?framework=${framework==="all"?"":framework}&domain=${encodeURIComponent(domain.name)}`} className="exec-domain-bar"><span>{domain.name}</span><div><i style={{width:`${value}%`}}/><b>{value}%</b></div><small>{domain.done} من {domain.total} ضابط مكتمل</small></Link>})}
          </div>
        </section>
        <section className="exec-visual-card exec-risk-card">
          <div className="exec-visual-heading"><div><span className="exec-eyebrow">تركيز الإدارة</span><h2>خريطة الأولويات</h2></div><Link href="/risks">سجل المخاطر ←</Link></div>
          <div className="exec-priority-map">
            <Link href="/tasks?filter=overdue" className={summary.overdue?"high":"low"}><strong>{summary.overdue}</strong><span>مهام متأخرة</span><small>{summary.overdue?"تحتاج تصعيدًا ومتابعة":"لا توجد متأخرات"}</small></Link>
            <Link href="/review" className={pending?"medium":"low"}><strong>{pending}</strong><span>أدلة بانتظار قرار</span><small>{pending?"قبول أو إرجاع الأدلة":"المراجعة مكتملة"}</small></Link>
            <Link href="/controls" className={summary.remaining?"medium":"low"}><strong>{summary.remaining}</strong><span>ضوابط لم تبدأ</span><small>تحتاج خطة تنفيذ ومالكًا</small></Link>
            <Link href="/controls" className={summary.inProgress?"medium":"low"}><strong>{summary.inProgress}</strong><span>ضوابط قيد التنفيذ</span><small>تابع التقدم والأدلة</small></Link>
          </div>
        </section>
      </section>
