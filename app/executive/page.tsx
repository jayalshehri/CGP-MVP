...
export default function ExecutivePage() {
  ...
  return <main dir="rtl" className="exec-page" aria-busy={loading}>
    <header className="exec-heading"><div><span className="exec-eyebrow">ملخص الإدارة</span><h1>اللوحة التنفيذية</h1><p>صورة واضحة للتنفيذ والتحقق، والأولويات التي تحتاج قرارًا.</p></div><div className="exec-tools cgp-print-action"><button onClick={refresh} disabled={loading}>{loading ? "جاري التحديث…" : "تحديث البيانات"}</button><button onClick={() => window.print()} disabled={!updated || loading || Boolean(error)}>طباعة الملخص</button></div></header>
