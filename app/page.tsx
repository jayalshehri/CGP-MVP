    ...
    <main dir="rtl" style={{minHeight:"100vh",background:"#f5f7f9",fontFamily:"Arial, sans-serif",color:"#0b1f33"}}>
      ...
      <div className="cgp-page-body" style={{display:"flex",minHeight:"calc(100vh - 86px)"}}>
        ...
        <section className="cgp-content" style={{flex:1,padding:"40px",minWidth:0}}>
          <WorkflowHeading title="لوحة المتابعة التشغيلية" description="متابعة حالة الالتزام والضوابط والأدلة والمهام." action={<button type="button" disabled={refreshing} onClick={()=>setRefreshKey(k=>k+1)} className="workflow-button">{refreshing?"جاري التحديث...":"تحديث البيانات"}</button>}/>
          ...
          <details className="cgp-insight-details"><summary><span><b>حالة الالتزام حسب الإطار</b><small>عرض تفاصيل الأطر الستة ونسبها</small></span><span aria-hidden="true">‹</span></summary><div className="cgp-responsive-grid cgp-framework-summary" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:"20px"}}>{stats?.domains.map(domain=><div key={domain.name} style={{background:"white",border:"1px solid #e2e7eb",borderRadius:"14px",padding:"24px"}}><h3>{domain.name}</h3><div className="cgp-domain-row"><div><strong>{domain.percentage}%</strong></div><progress max={100} value={domain.percentage} aria-label={`الالتزام في ${domain.name}`}/><p>{domain.done} من {domain.total} ضابط مكتمل</p></div></div>)}</div></details>
